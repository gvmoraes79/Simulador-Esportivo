...
import { RotateCcw, Target, MapPin, Flag, ExternalLink, Lightbulb, TrendingUp, Download, Share2 } from 'lucide-react';
...
const ResultView: React.FC<ResultViewProps> = ({ result, onReset }) => {
  ...
  const exportPDF = async () => {
    ...
    pdf.save(`SportSim_Report_${result.homeTeam.name}_vs_${result.awayTeam.name}.pdf`);
  };
  ...
  return (
    <div className="space-y-10 animate-fade-in-up pb-20">
      <div ref={reportRef} className="space-y-10 p-2 md:p-6 rounded-[3rem]">
        {/* Placar Principal */}
        <div className="bg-slate-900 border border-slate-800 rounded-[3rem] p-8 md:p-12 shadow-2xl relative overflow-hidden text-center">
           ...
           <div className="grid grid-cols-1 md:grid-cols-3 items-center gap-8">
              <div className="space-y-2">
                 <h2 className="text-2xl md:text-3xl font-black text-white uppercase tracking-tighter">{result.homeTeam.name}</h2>
              ...
              <div className="flex flex-col items-center gap-2">
                 <div className="bg-slate-950 border-2 border-slate-800 rounded-3xl px-6 py-4 flex items-center gap-4 md:gap-6 shadow-inner">
                    <span className="text-5xl md:text-7xl font-black text-white tabular-nums">{result.predictedScore.home}</span>
                    <div className="w-[2px] h-10 md:h-12 bg-slate-800"></div>
                    <span className="text-5xl md:text-7xl font-black text-white tabular-nums">{result.predictedScore.away}</span>
                 </div>
                 <span className="text-[8px] md:text-[10px] text-slate-600 font-bold uppercase tracking-[0.3em]">AI Prediction</span>
              </div>

              <div className="space-y-2">
                 <h2 className="text-2xl md:text-3xl font-black text-white uppercase tracking-tighter">{result.awayTeam.name}</h2>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
           <div className="md:col-span-2 bg-slate-900 border border-slate-800 p-8 md:p-10 rounded-[2.5rem] space-y-6">
           ...
           <div className="bg-slate-900 border border-slate-800 p-8 md:p-10 rounded-[2.5rem] space-y-8">
      ...
      <div className="flex flex-col sm:flex-row justify-center gap-4 pb-12">
        <button onClick={exportPDF} className="bg-emerald-600 hover:bg-emerald-500 text-white font-black px-10 py-4 rounded-2xl flex items-center justify-center gap-3 transition-all shadow-xl shadow-emerald-900/20">
           <Download size={20} /> Exportar Relatório (PDF)
        </button>
        <button onClick={onReset} className="bg-slate-800 hover:bg-slate-700 text-white font-black px-10 py-4 rounded-2xl flex items-center justify-center gap-3 transition-all">
